...
internal class BeforeDepartureTracking : TrackingStrategy
{
    ...
    protected internal override string GetMessage()
    {
        return
        $"""
        Tracking of bus {BusName} started at {TrackingStartedTime.AddHours(-DatetimeProvider.GetUtcDifference()).ToShortTimeString()}. It has not yet reached the first stop.
