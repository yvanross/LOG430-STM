...
public class UpdateStaticGtfsHandler : ICommandHandler<UpdateStaticGtfsCommand>
{
    ...
    public UpdateStaticGtfsHandler(
    ...
    public async Task Handle(UpdateStaticGtfsCommand command, CancellationToken cancellation)
    {
        try
        {
            ...
            _logger.LogInformation("Updating Aggregates, this is a long operation... (Between 1-5 minutes on most systems)");

            await _stopWriteRepository.UpdateAllAsync(stops.ToList());

            await _tripWriteRepository.UpdateAllAsync(trips.ToList());

            //using bulk updates so save changes is not needed but it still dispatches the domain events
            await _unitOfWork.SaveChangesAsync();
        }
        catch (Exception e)
        {
            _logger.LogError(e, "Error while updating static gtfs data");
            throw;
        }
    }
}